...
<div class="d-flex justify-content-between align-items-center mb-3">
    <h4 class="fw-bold" style="color:var(--brand);"><i class="fas fa-edit me-2"></i>Edit Invoice <?php echo e($invoice->invoice_number); ?></h4>
    <div>
        <a href="<?php echo e(route('invoices.pdf', $invoice)); ?>" class="btn btn-outline-danger"><i class="fas fa-file-pdf"></i> PDF</a>
        <a href="<?php echo e(route('invoices.show', $invoice)); ?>" class="btn btn-outline-secondary"><i class="fas fa-arrow-left me-1"></i>Back</a>
    </div>
</div>
...
<div class="card shadow-sm mb-3" style="border:1px solid #dee2e6;">
    <div class="card-body p-3">
        ...
        <div class="row g-2 align-items-center">
            <div class="col-auto">
                <input type="date" id="tt-from" class="form-control form-control-sm" value="<?php echo e(date('Y-m-01')); ?>">
            </div>
            <div class="col-auto">
                <span class="mx-1 text-muted">to</span>
            </div>
            <div class="col-auto">
                <input type="date" id="tt-to" class="form-control form-control-sm" value="<?php echo e(date('Y-m-d')); ?>">
            </div>
            <div class="col-auto">
...
<div class="card shadow-sm mb-4" style="border:1px solid #dee2e6;">
    <div class="card-body p-4">
        <form method="POST" action="<?php echo e(route('invoices.update', $invoice)); ?>" id="invoiceForm">
            <?php echo csrf_field(); ?>
            <?php echo method_field('PUT'); ?>
            <?php echo $__env->make('invoices.partials.document-styles', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
            <?php echo $__env->make('invoices.partials.document', ['editable' => true, 'invoice' => $invoice], array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>

            <div class="mt-4 pt-3 border-top text-end">
                <button type="submit" class="btn btn-brand btn-lg px-5"><i class="fas fa-save me-2"></i>Update Invoice</button>
            </div>
        </form>
    </div>
</div>
...
<script>
let itemCount = <?php echo e(count(old('items', $invoice->items))); ?>;
...
$('#tt-import-btn').on('click', function() {
    ...
    fetch(`<?php echo e(route('time-tracker.summary')); ?>?from=${from}&to=${to}`)
        .then(r => r.json())
        .then(data => {
...
<?php echo $__env->make('invoices.partials.invoice-form-scripts', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
<?php $__env->stopPush(); ?>

<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH E:\wamp64\www\invoice-system-server\resources\views/invoices/edit.blade.php ENDPATH**/ ?>